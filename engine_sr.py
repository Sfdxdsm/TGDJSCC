...
def train_one_epoch(args, model, train_loader, optimizer, epoch, logger):
    ...
    if args.trainset == 'DIV2K':
        for batch_idx, (input, HR_image) in enumerate(train_loader):
            ...
            HR_image = HR_image.to(args.device)
            recon_image, mse, loss_G, CBR, SNR = model(input, HR_image)
            loss = loss_G
            optimizer.zero_grad()
            loss.backward()
            ...
            if mse.item() > 0:
                ...
                msssim = 1 - CalcuSSIM(HR_image, recon_image.clamp(0., 1.)).mean().item()
...
def test(args, model, test_loader, logger):
    ...
    for i, SNR in enumerate(multiple_snr):
        with torch.no_grad():
            for batch_idx, (input, HR_image) in enumerate(test_loader):
                start_time = time.time()
                input = input.to(args.device)
                HR_image = HR_image.to(args.device)
                recon_image, mse, loss_G, CBR, SNR = model(input, HR_image, SNR)
                elapsed.update(time.time() - start_time)
                cbrs.update(CBR)
                snrs.update(SNR)
                if mse.item() > 0:
