...
from util.modules import UpSample, Block, AdaptiveModulator, DownSample
...
class Encoder(timm.models.vision_transformer.VisionTransformer):
    ...
    def __init__(self, C, selected_nodes_num=16, window_size=4, multiple_snr='1,4,7,10,13', chan_type='awgn', **kwargs):
        ...
        # for i in range(num_merge):
        #     self.DS_list.append(DownSample(downsample=math.floor(math.log(window_size, 4)), dim=embed_dim, out_dim=embed_dim))
    ...
    def forward(self, x, chan_param=1, task_id=0):
        ...
        latent_tokens = self.latent_norm(node_feats[:, :self.latent_tokens_num, :])
        ...
        return selected_feats, latent_tokens
...
class Decoder(timm.models.vision_transformer.VisionTransformer):
    ...
    def __init__(self, C, selected_nodes_num=64, **kwargs):
...
class Net(nn.Module):
    ...
    def forward(self, input_image, chan_param=None):
        ...
        mse = self.squared_difference(input_image * 255., recon_image.clamp(0., 1.) * 255.).mean()
        loss_G = self.distortion_loss.forward(input_image, recon_image.clamp(0., 1.)).mean()
...
def dec_base_patch2_embed256(**kwargs):
    model = Decoder(
        img_size=32, patch_size=2, embed_dim=256, depth=3, num_heads=8, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
