...
from util.pos_embed import interpolate_pos_embed, get_2d_sincos_pos_embed, interpolate_latent_pos_embed
from util.misc import NativeScalerWithGradNormCount as NativeScaler
import torchvision.datasets as datasets
from engine_finetune import train_one_epoch, evaluate, evaluate_with_channel
...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument('--model', default='classifier_base_patch2_embed256', type=str, metavar='MODEL',
    ...
    parser.add_argument('--train_data_path', default='/home/csudz/Desktop/dsm/TJSCC/dataset/cifar100', type=str,
                        help='dataset path')
    parser.add_argument('--val_data_path', default='/home/csudz/Desktop/dsm/TJSCC/dataset/cifar100', type=str,
                        help='dataset path')
    # parser.add_argument('--train_data_path', default='/home/csudz/Desktop/dsm/TJSCC/dataset/SVHN', type=str,
    #                     help='dataset path')
    # parser.add_argument('--val_data_path', default='/home/csudz/Desktop/dsm/TJSCC/dataset/SVHN', type=str,
    #                     help='dataset path')
...
def main(args):
    ...
    # dataset_train = datasets.SVHN(args.train_data_path, split='train', download=True,
    #                                   transform=transform_train)
    # dataset_val = datasets.SVHN(args.train_data_path, split='test', download=True,
    #                                 transform=transform_test)
    ...
    model = models_classifier.__dict__[args.model](
        drop_path_rate=args.drop_path,
        n_classes=args.nb_classes,
        multiple_snr=args.multiple_snr,
        chan_type=args.chan_type,
        C=args.C,
    ...
    if args.finetune and not args.eval:
        ...
        if args.finetune_from_jscc:
            checkpoint_model = collections.OrderedDict(
                [(k.replace('encoder.', ''), v) if k.split('.')[0] == 'encoder' else (k, v) for k, v in
        ...
        # or key.startswith('DownSample') or key.startswith('fc_norm')
    ...
    if args.eval:
        model.eval()
        test_stats = evaluate_with_channel(data_loader_val, model, device, args.multiple_snr)
        print(f"Accuracy of the network on the {len(dataset_val)} test images:")
        print(f"acc1: {test_stats['acc1']}, acc5: {test_stats['acc5']}")
        exit(0)
